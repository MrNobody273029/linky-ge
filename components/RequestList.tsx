        ...
        {items.map((r) => {
          ...
          return (
            <div key={r.id} className={cn('relative', isOpen ? 'bg-card/20' : '')}>
              <button onClick={() => setOpenId(isOpen ? null : r.id)} className="w-full text-left hover:bg-card/40">
                <div className="grid grid-cols-1 gap-2 px-4 py-4 md:grid-cols-[1.2fr_0.7fr_0.9fr_110px] md:items-center md:gap-3 md:py-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-muted md:hidden break-all">{truncate(r.productUrl, 80)}</div>
                  </div>
                ...
                <div className="border-t border-border bg-card/20 px-4 py-4">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-[220px_1fr]">
                    <div className="relative h-44 w-full overflow-hidden rounded-2xl border border-border bg-card/40">
                      {r.offer?.imageUrl ? (
                        ...
                        <div className="flex h-full w-full items-center justify-center text-sm text-muted">{t('noImage')}</div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <a href={r.productUrl} target="_blank" rel="noreferrer" className="text-xs text-muted hover:underline">
                      ...
                      {r.offer?.note ? (
                        <div className="rounded-2xl border border-border bg-card/30 p-3 text-sm text-muted">{r.offer.note}</div>
                      ) : null}
                  ...
                  {payOpenFor === r.id ? (
                    <Modal title={t('pay30Title')} subtitle={t('pay30Subtitle')} onClose={() => setPayOpenFor(null)}>
                      <div className="grid grid-cols-2 gap-3">
                        <Info label={t('linkyPrice')} value={r.offer ? `${r.offer.linkyPrice} ${r.currency ?? 'GEL'}` : '—'} />
                        <Info label={t('pay30Amount')} value={r.offer ? `${pay30Amount} ${r.currency ?? 'GEL'}` : '—'} />
                      </div>
                  ...
                  {/* DECLINE MODAL */}
                  {declineOpenFor === r.id ? (
                    <Modal title={t('declineTitle')} subtitle={t('declineSubtitle')} onClose={() => setDeclineOpenFor(null)}>
                      <textarea
...
function Step({ active, icon, label }: { active: boolean; icon: React.ReactNode; label: string }) {
  return (
    <div className={cn('flex items-center gap-2 rounded-xl border border-border px-3 py-2', active ? 'bg-card/50' : 'bg-card/20 text-muted')}>
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/50 p-2 md:items-center md:p-6" onClick={onClose}>
